...
colnames(Result.AUC) <- c("rou", "Beta0" , "AUC2", "Emp.sd_AUC2", "Asy.sd_AUC2")

# Initialize variables to store AUC values and standard deviation of AUC values for each replicate
AUC2_values <- numeric(replicate)
Asy_sd_AUC2<-numeric(replicate)
...
for (ci in 1:replicate) {
  ...
  #generate case-control data Phase II sample Balanced
  ...
  AUC2_values[ci] <- auc(fpr.mle, tpr.mle)
  ...
  Asy_sd_AUC2[ci]<-sqrt(var_auc)
  
}

Result.AUC[,1]<-rou
Result.AUC[,2]<-beta0
Result.AUC[,3]<-mean(AUC2_values)
Result.AUC[,4]<-sd(AUC2_values)
Result.AUC[,5]<-mean(Asy_sd_AUC2)
