...
colnames(Result.AUC) <- c("rou",  "AUC1", "Emp.sd_AUC1", "Asy.sd_AUC1", "count_y0", "count_y1", "Phase2_Zcase","Phase2_Znoncase")
...
dat_case_counts <- numeric(replicate)
dat_con_counts <- numeric(replicate)
...
for (ci in 1:replicate) {
  ...
  dat_case_counts[ci] <- sum(dat$y == 1)
  dat_con_counts[ci] <- sum(dat$y == 0)
    
   #generate case-control data Phase II  SRS
   #set.seed(ci)
  dat2_indices <- sample(1:n, size = round(n * 0.15 * 3))#默认无放回抽样
  dat2 <- dat[dat2_indices, ]
  
   # Count y=1 and y=0 in the Phase 2 sample
...
Result.AUC[,5] <- mean(dat_case_counts)
Result.AUC[,6] <- mean(dat_con_counts)
Result.AUC[,7] <- mean(Phase2_case_counts)
Result.AUC[,8] <- mean(Phase2_noncase_counts)
